...
import pandas as pd
from gsheets import update_sheet, insert_sheet, get_gsheets_columns, get_delimiter
...
def add_to_dataset(tab, connection, add_df, add_pub_df):
    project_df = st.session_state.get("project_df", pd.DataFrame())
    pub_df = st.session_state.get("pub_df", pd.DataFrame())
